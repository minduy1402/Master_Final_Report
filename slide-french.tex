...
\begin{frame}{Combler les Lacunes : Deux Approches Complémentaires}
    \begin{columns}[T]
        ...
        \begin{block}{Traiter la Lacune Comportementale}
            \textbf{Approches Courantes :}
            \begin{itemize}
                \item Prompt engineering avancé
...
\begin{frame}{Problème 1 : Intégration de Connaissances Externes}
    \begin{block}{Cas d'usage : Construction d'un système de QA spécifique au domaine pour documents financiers et médicaux}
        \begin{itemize}
            ...
            \item \textbf{Challenges :} Documents multi-modal, longs, de structures complexes. 
        \end{itemize}
    \end{block}
...
\begin{frame}{Pipeline RAG Simple - Approche de Référence}
    \begin{columns}[c]
        ...
        \begin{column}{0.48\textwidth}
            ...
            \begin{itemize}
                \item \textbf{Parsing :} PyPDF pour l'extraction de texte
                \item \textbf{Chunking :} Taille fixe (512 tokens/chunk, 50 de chevauchement)
                \item \textbf{Embedding :} all-MiniLM-L6-v2 
                \item \textbf{Vector Store :} ChromaDB avec recherche dense
...
\begin{frame}{Pourquoi le RAG Simple Échoue : Analyse Quantitative}
    ...
    \begin{itemize}
        \item \textbf{Lost in the middle :} La précision du LLM diminue lorsque le passage de ground truth n'est pas en position élevée dans les top-k résultats
...
\begin{frame}{Pipeline RAG Multimodal Avancé}
    \begin{columns}[c]
...
\begin{frame}{Offline Indexing : Préservation de la Structure}
    \begin{figure}
        \centering
...
\begin{frame}{Offline Indexing : Exemple de Préservation de Structure}
    \begin{columns}[c]
...
\begin{frame}{Offline Indexing : Résumé Multimodal}
    \begin{figure}
        \centering
...
\begin{frame}{Offline Indexing : Architecture Parent-Enfant}
    \begin{figure}
        \centering
...
\begin{frame}{Offline Indexing : Modèle d'Embedding Multilingue}
    \begin{figure}
        \centering
...
\begin{frame}{Résultats : Amélioration Supérieure de la Précision}
    \begin{table}[H]
        ...
        \begin{tabular}{|l|c|c|c|}
            \hline
            \textbf{Pipeline} & \textbf{Exact.} & \textbf{Pert.}  & \textbf{Préc.} \\
            \hline
            RAG Simple & 0.648 & 0.788 & 0.654 \\
            \hline
            RAG Avancé & \textbf{0.976} & \textbf{0.972}  & \textbf{0.960} \\
            \hline
            \textit{Amélioration} & \textit{+32.8\%} & \textit{+18.4\%} & \textit{+30.6\%} \\
            \hline
        \end{tabular}
    \end{table}
...
\begin{frame}[plain]
    \begin{center}
        ...
        {\Large Adaptation Comportementale pour Petits Modèles}
        \vspace{0.3cm}
...
\begin{frame}{Problème 2 : Adaptation Comportementale pour Petits Modèles}
    ...
    \vspace{0.3cm}
    \textbf{Approche Naïve :} Prompt engineering avec de grands LLM (GPT-4, Claude-3) via API
    
    \vspace{0.3cm}
    \textbf{Pourquoi Prompt engineering Seule Est Insuffisante :}
    \begin{itemize}
...
\begin{frame}{Pipeline de Distillation : Génération de Données d'Entraînement Synthétiques}
    \begin{figure}
        \centering
        \includegraphics[width=0.8\textwidth]{figures/distillation_pipeline.png}
    \end{figure}
    \footnotesize
...
\begin{frame}{Sélection du Modèle : Pourquoi Qwen2.5-1.5B-Instruct ?}
    ...
    \begin{enumerate}
        ...
        \item Capacité de suivi d'instructions
    \end{enumerate}
\end{frame}
...
\begin{frame}{Résultats du Fine-tuning : Évaluation Complète}
    ...
    \begin{itemize}
        ...
        \item Qwen2.5-1.5B fine-tuné surpasse GPT-4o avec prompts sur le style, l'empathie et le workflow
    \end{itemize}
\end{frame}
...
\begin{frame}{Contributions Clés}
    ...
    \begin{itemize}
        ...
        \item Qwen2.5-1.5B fine-tuné surpasse GPT-4o avec prompts sur les métriques comportementales
...
\begin{frame}{Directions Futures}
    ...
    \begin{itemize}
        ...
        \item Optimisation pour le déploiement en edge devices (quantization)
    \end{itemize}
    ...
    \vspace{0.3cm}
    \textbf{\color{MainNavy}Key takeaways}
    \begin{itemize}
        \item Les modèles de langage offrent de nombreuses possibilités d'exploitation—le choix de la méthode d'adaptation et d'optimisation doit être guidé par les exigences spécifiques du problème
    \end{itemize}
\end{frame}
